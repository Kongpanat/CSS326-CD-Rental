	...
	if(isset($_POST['f_add'])){
		...
		if (empty($title)) {
			...
			echo ("<script LANGUAGE='JavaScript'>
					window.alert('Enter film title');	
					</script>");
        }
		if (empty($releasey)) {
			$x=1;
			echo ("<script LANGUAGE='JavaScript'>
					window.alert('Enter film release year');	
					</script>");
        }
		if (empty($genre)) {
			$x=1;
			echo ("<script LANGUAGE='JavaScript'>
					window.alert('Enter film genre');	
					</script>");
        }
		if (empty($length)) {
			$x=1;
			echo ("<script LANGUAGE='JavaScript'>
					window.alert('Enter the length of film');	
					</script>");
        }
		if (empty($rentalrate)) {
			$x=1;
			echo ("<script LANGUAGE='JavaScript'>
					window.alert('Enter Rental rate');	
					</script>");
        }
		if (empty($detail)) {
			$x=1;
			echo ("<script LANGUAGE='JavaScript'>
					window.alert('Add Detail');	
					</script>");
        }
		if($result) { // Title already used or not
                if ($result['title'] === $title){
					$x=1;
					echo ("<script LANGUAGE='JavaScript'>
					window.alert('This Movie already exist');	
					</script>");
                }
        }
		if($x==1){
		...
		if($x==0){
			...
			VALUES ('1','$title','$releasey','30','$rentalrate','$length','$rating','9')";
